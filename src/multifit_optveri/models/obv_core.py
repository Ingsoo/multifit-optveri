...
# globals `gp` and `GRB`, then call `build_obv_model`.
# If you are reading the code with the paper open, the high-level map is:
# 1. `build_obv_model`: Section 4 base model.
...
# `x`, `q`, and `s` are backend tupledict-like objects because they are indexed
# by two coordinates and frequently used inside generator expressions.


class BackendApiUnavailableError(RuntimeError):
    """Raised when the shared builder is called without an algebraic backend API."""


GurobiUnavailableError = BackendApiUnavailableError
...
@dataclass
class BuiltObvModel:
    """Bundle the built model with a coarse dimension summary."""
...
def _as_float(value: Fraction) -> float:
    """Convert an exact Fraction into the float form expected by backend APIs."""
